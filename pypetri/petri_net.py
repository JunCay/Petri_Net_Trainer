...
class ColoredPetriNet():
    def __init__(self, name):
        ...
        self.is_robot = True
        # self.reward_dict = {'progress': 10, 'fire': 2, 'unready_fire': -10, 'on_fire_fire': -20, 'duplicate_fire': -20, 'idle': -self.dt, 'fire_time_penalty': -self.dt, 'consumption_panelty': -0.1, 'same_gesture': 40, 'height_cost':-0.4}
        self.reward_dict = {'progress': 1, 'fire': 0.5, 'unready_fire': -10, 'on_fire_fire': -20, 'duplicate_fire': -20, 'idle': -self.dt, 'fire_time_penalty': -self.dt*5, 'consumption_panelty': 1, 'same_gesture': 0, 'height_cost':0}
        self.last_fire = None
        self.is_pcpn = False
    ...
    def set_dt(self, dt):
        self.dt = dt
        self.reward_dict['idle'] = -self.dt*5
        self.reward_dict['fire_time_penalty'] = -self.dt
    ...
    def init_by_csv(self, path):
        ...
        with open(path, 'r', encoding='utf-8-sig') as f:
            ...
            for row in reader:
                # print(f'row: {row}')
                row = {k.strip(): v.strip() if v!=None else '' for k, v in row.items()}
                command = row['command']
                args_str = re.sub(r"'(\w+)':", r'"\1":', row['args']).replace("'", '"').replace("[", "{").replace("]", "}")
                args_list = ast.literal_eval(args_str)
                args = dict()
                # args = []
                for arg in args_list.keys():
                    try:
                        a = ast.literal_eval(args_list[arg])
                        # args.append(a)
                        args[arg] = a
                    except:
                        # args.append(arg)
                        args[arg] = args_list[arg]
                # print(args)
                if command == 'MADP':
                    # Add Place Node
                    ...
                    # Add Arc
                    self.add_arc(self.name_node[args['node_in_name']], self.name_node[args['node_out_name']])
                elif command == 'MAST':
                    # Set target marking
                    self.name_node[args['place_name']].set_target_marking(args['target_token_num'])
    ...
    def init_by_json(self, path):
        with open(path, 'r', encoding='utf-8') as f:
            ...
            for row in commands:
                ...
                args_list = row['args']
                args = {}
                for arg_key, arg_value in args_list.items():
                    try:
                        # 解析为字典或其他结构
                        parsed_value = ast.literal_eval(arg_value) if isinstance(arg_value, str) else arg_value
                        args[arg_key] = parsed_value
                    except:
        ...
        if arc.direction == 'PtoT':
            ...
            for k in arc.annotation.keys():
                if arc.node_in.marking[k] - arc.annotation[k] >= 0:
                    return True
            return False
        if self.is_pcpn and arc.direction == 'TtoP':
    ...
    def on_fire_transition(self, transition:Transition):
        ...
        if transition.consumption >= 0.001:
            ...
            if transition.target_height != -1.0:
                self.current_height = transition.target_height
        ...
        for i, place in enumerate(self.places.values()):
            for mt in marking_types:
                if mt in place.marking.keys():
                    place_state[i, marking_types.index(mt)] = place.marking[mt] / place.capacity
    ...
    def get_transition_neural_state(self):
        ...
        trans_state = np.zeros((len(self.transitions)+1, 2))
        for i, transition in enumerate(self.transitions.values()):
            if transition.status == 'ready':
                trans_state[i, 0] = 1.0
            else:
            ...
            if trans == None:
                ...
                if not self.transition_ready_check(trans):
                    ...
                    if self.on_fire_transition(trans):
                        ...
                        reward_dict['consumption_panelty'] = self.reward_dict['consumption_panelty'] * trans.this_consumption
                        
                        if self.is_robot:
                            if (not trans.is_non_gesture) and trans.is_same_gesture(self.current_gesture):
                                reward_dict['same_gesture'] = self.reward_dict['same_gesture']
                            reward_dict['height_cost'] = self.reward_dict['height_cost'] * trans.get_height_difference(self.current_height)
                    else:
